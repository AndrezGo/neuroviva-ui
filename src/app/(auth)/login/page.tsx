'use client';

import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import Link from 'next/link';
import { BackButton } from '@/presentation/ui/BackButton';
import { TextField } from '@/presentation/ui/TextField';
import { PasswordField } from '@/presentation/ui/PasswordField';
import { Button } from '@/presentation/ui/Button';
import { Divider } from '@/presentation/ui/Divider';
import { useSignIn } from '@/application/auth/useSignIn';
import { useGoogleAuth } from '@/application/auth/useGoogleAuth';
import { signInSchema, type SignInFormValues } from '@/application/auth/authSchemas';
import { routes } from '@/core/routing/routes';

export default function LoginPage() {
  const { signIn, isLoading, error } = useSignIn();
  const { signInWithGoogle, isLoading: isGoogleLoading } = useGoogleAuth();

  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<SignInFormValues>({
    resolver: zodResolver(signInSchema),
    defaultValues: {
      email: '',
      password: '',
    },
  });

  const onSubmit = handleSubmit(async (values) => {
    await signIn(values);
  });

  return (
    <div className="flex flex-1 flex-col px-6 py-8">
      {/* Top navigation */}
      <div className="animate-fade-in" style={{ animationDelay: '0ms' }}>
        <BackButton />
      </div>

      {/* Header */}
      <div className="animate-fade-up mt-8" style={{ animationDelay: '60ms' }}>
        <h1 className="text-3xl font-black tracking-tight text-brand-dark">Hola de nuevo</h1>
        <p className="mt-2 text-base text-gray-text">Inicia sesión para continuar con el cuidado.</p>
      </div>

      {/* Form */}
      <form
        onSubmit={onSubmit}
        noValidate
        className="animate-fade-up mt-8 flex flex-col gap-5"
        style={{ animationDelay: '120ms' }}
      >
        <TextField
          label="Correo electrónico"
          type="email"
          autoComplete="email"
          placeholder="example@email.com"
          error={errors.email?.message}
          {...register('email')}
        />

        <div className="flex flex-col gap-1.5">
          <PasswordField
            label="Contraseña"
            autoComplete="current-password"
            placeholder="Tu contraseña"
            error={errors.password?.message}
            {...register('password')}
          />
          <div className="flex justify-end">
            <Link
              href={routes.forgotPassword()}
              className="text-sm font-semibold text-brand-primary hover:text-brand-primary-dark transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-1 rounded"
            >
              ¿Olvidaste tu contraseña?
            </Link>
          </div>
        </div>

        {/* Form-level error */}
        {error && (
          <div
            role="alert"
            className="rounded-2xl bg-red-50 border border-red-200 px-4 py-3 text-sm font-medium text-red-600"
          >
            {error}
          </div>
        )}

        <Button
          type="submit"
          variant="primary"
          size="lg"
          fullWidth
          isLoading={isLoading}
        >
          Iniciar sesión
        </Button>
      </form>

      {/* Divider + Google */}
      <div
        className="animate-fade-up mt-6 flex flex-col gap-4"
        style={{ animationDelay: '180ms' }}
      >
        <Divider />
        <Button
          type="button"
          variant="secondary"
          size="lg"
          fullWidth
          isLoading={isGoogleLoading}
          onClick={() => signInWithGoogle()}
        >
          <GoogleIcon />
          Continuar con Google
        </Button>
      </div>

      {/* Footer */}
      <p
        className="animate-fade-up mt-auto pt-8 text-center text-sm text-gray-text"
        style={{ animationDelay: '240ms' }}
      >
        ¿No tienes cuenta?{' '}
        <Link
          href={routes.register()}
          className="font-semibold text-brand-primary hover:text-brand-primary-dark transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-1 rounded"
        >
          Crear cuenta
        </Link>
      </p>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      className="h-5 w-5"
      aria-hidden="true"
      focusable="false"
    >
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}
