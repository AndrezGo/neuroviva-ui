...
import { adminTabItems } from '@/presentation/admin/navigation';
...
/**
 * Fixed 240px sidebar for the Admin desktop layout.
 * Uses the brand gradient background (--gradient-brand) defined in globals.css.
 * Mirrors DoctorDesktopSidebar — purely presentational, no hooks, no API calls.
 * All tabs are always visible (no committee filtering).
 */
export function AdminDesktopSidebar({ activeTab }: AdminDesktopSidebarProps) {
  return (
    <aside
    ...
    >
      ...
      <nav aria-label="Navegación de escritorio" className="flex flex-1 flex-col gap-1 overflow-y-auto px-3 py-4">
        {adminTabItems.map(({ tab, label, href, Icon }) => {
