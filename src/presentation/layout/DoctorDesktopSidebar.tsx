...
import { doctorTabItems } from '@/presentation/doctor/navigation';
...
/**
 * Fixed 240px sidebar for the Doctor desktop layout.
 ...
 */
export function DoctorDesktopSidebar({
  activeTab,
  isScientificCommittee = false,
}: DoctorDesktopSidebarProps) {
  const visibleItems = doctorTabItems.filter(
  ...
  return (
    ...
    >
      ...
      <nav aria-label="Navegación de escritorio" className="flex flex-1 flex-col gap-1 overflow-y-auto px-3 py-4">
        {visibleItems.map(({ tab, label, href, Icon }) => {
