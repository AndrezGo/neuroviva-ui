...
interface PatientDesktopHeaderProps {
  firstName: string;
  email: string;
  onSignOut: () => void;
...
/**
 * Fixed top header for the Patient desktop layout.
 * Sits to the right of the 240px sidebar (ml-60).
 * No notification bell — patients have no notifications system.
 * Purely presentational — no hooks, no API calls.
 */
export function PatientDesktopHeader({
  firstName,
  email,
  onSignOut,
  isSigningOut,
}: PatientDesktopHeaderProps) {
  return (
    <header
      className={cn(
    ...
    >
      {/* ── Right slot: account menu only (no bell for patients) ── */}
